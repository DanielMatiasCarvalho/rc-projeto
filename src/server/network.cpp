...
UdpServer::UdpServer(std::string port) {
    ...
    int err = getaddrinfo(NULL, port.c_str(), &_hints,
...
void UdpServer::send(std::stringstream &message) {
    char messageBuffer[SOCKETS_MAX_DATAGRAM_SIZE_CLIENT];  // The message buffer
...
TcpServer::TcpServer(std::string port) {
    ...
    int n = getaddrinfo(NULL, port.c_str(), &_hints,
                        &_res);  // Get the address info
