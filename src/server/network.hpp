 ...
 *
 * This file contains the declaration of the UdpServer, TcpServer, TcpSession,
...
/**
 * @class UdpServer
 * @brief Represents a UDP server that can send and receive data over the
 * network.
 */
class UdpServer {
  private:
...
class TcpServer {
  ...
  public:
     ...
     *
     * This function is responsible for accepting incoming connections from
     * clients. It should be called in a loop to continuously accept new
     * connections.
     */
...
/**
 * @class TcpSession
 * @brief Represents a TCP session that can send and receive data over the
 * network.
 */
class TcpSession {
  private:
  ...
  public:
    int _fd;
    /**
     * @brief Constructs a TcpSession object with the specified file descriptor,
     * addres info hints and res.
 ...
 * @brief Exception thrown when there is a problem reading/writing from/to a
 * socket.
 */
