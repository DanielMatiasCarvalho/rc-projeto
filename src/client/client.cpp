...
int main(int argc, char **argv) {
    Client client(argc, argv); // Create object Client, parsing command line arguments in the construction
    ...
    while (!client._toExit) { // While the client is not exiting
        try {
            std::string command = terminal.readLine("> "); //Read a line from the terminal
            manager.readCommand(command, client); //Reads the command  and its arguments, sending it to the correct handler
        } catch (CommandException const &e) { // If an exception is thrown, print the error message
            std::cout << e.what() << std::endl;
        } catch (ProtocolViolationException const &e) {//If there is an error in the protocol
            std::cout << e.what() << std::endl;
        } catch (ProtocolMessageErrorException const &e) { //If the server sends an error message
            std::cout << e.what() << std::endl;
        } catch (TimeoutException const &e) { // If the server doesn't respond in time, print the error message
            std::cout << e.what() << std::endl;
...
bool User::isLoggedIn() {
    return ((_username.length() != 0) && (_password.length() != 0)); //If the username and password are not empty, the user is logged in
}
...
void Client::processRequest(ProtocolCommunication &comm) {
    std::stringstream reqMessage = comm.encodeRequest(), resMessage; //Encode the request
...
void Client::writeFile(std::string fName, std::stringstream &content) {
    ...
    std::ofstream file(_downloadPath + fName); //Create a file with the given name
    ...
    while (n != 0) { //While there is content to be read, loops and writes to the file
        file.write(buffer, n);
...
void Client::assureDirectory() {
    if (mkdir(_downloadPath.c_str(), 0777) == -1) { //If the directory doesn't exist, create it
        if (errno != EEXIST) {
