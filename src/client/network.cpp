/**
 * @file network.cpp
 * @brief Implementation of the UdpClient and TcpClient classes for network
 * communication.
 */
...
UdpClient::UdpClient(std::string hostname, std::string port) {
    ...
    int err = getaddrinfo(hostname.c_str(), port.c_str(), &_hints,
                          &_res);  // Get the address info
...
std::stringstream UdpClient::receive() {
    char messageBuffer[SOCKETS_MAX_DATAGRAM_SIZE_CLIENT +
                       1];  // Create a buffer to store the message, size+1 to
                            // check if the server sent a message bigger than it
                            // is supposed
    ...
    message.write(messageBuffer,
                  (std::streamsize)n);  // Write the message to a stringstream
...
TcpClient::TcpClient(std::string hostname, std::string port) {
    ...
    int n = getaddrinfo(hostname.c_str(), port.c_str(), &_hints,
                        &_res);  // Get the address info
...
void TcpClient::send(std::stringstream &message) {
    ...
    while (n != 0) {  // While there is content to be read, loops and writes to
                      // the socket
...
std::stringstream TcpClient::receive() {
    ...
    ssize_t n =
        read(_fd, messageBuffer, SOCKETS_TCP_BUFFER_SIZE);  // Read the message
