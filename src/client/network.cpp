...
std::stringstream UdpClient::receive() {
    char messageBuffer
        [SOCKETS_MAX_DATAGRAM_SIZE_CLIENT +
         1];  //Create a buffer to store the message, size+1 to check if the server sent a message bigger than it is supposed
...
void TcpClient::send(std::stringstream &message) {
    ...
    while (
        n !=
        0) {  //While there is content to be read, loops and writes to the socket
