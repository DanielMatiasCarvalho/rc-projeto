/**
 * @brief This file contains the implementation of various command classes used
 * in the client application. The CommandManager class is responsible for
 * registering and executing commands. Each command class implements the
 * handle() function to perform the specific command logic. The command classes
 * handle different actions such as login, logout, auction management, bidding,
 * and listing auctions. It performs the necessary validations and communicates
 * with the server through the client object to execute the command. If any
 * validation fails or an error occurs during command execution, appropriate
 * exception messages are thrown.
 */
#include "command.hpp"
...
void LoginCommand::handle(std::vector<std::string> args, Client &receiver) {
    ...
    if (receiver._user.isLoggedIn()) {  // If the user is already logged in,
                                        // show a message
    ...
    receiver.processRequest(
        loginCommunication);  // Send the request to the server, receiving its
                              // response
    ...
    if (loginCommunication._status == "OK") {
        ...
        // If the response is NOK, show a message for incorrect match (username
        // and/or password)
        Message::WrongPassword();
    } else if (loginCommunication._status == "REG") {
        // If the response is REG, show a register sucess message and login the
        // user
...
void LogoutCommand::handle(std::vector<std::string> args, Client &receiver) {
    ...
    receiver.processRequest(
        logoutCommunication);  // Send the request to the server, receiving its
                               // response
...
void UnregisterCommand::handle(std::vector<std::string> args,
    ...
    // Create an UnregisterCommunication object and set the username and
    // password
    ...
    receiver.processRequest(
        unregisterCommunication);  // Send the request to the server, receiving
                                   // its response
...
void OpenCommand::handle(std::vector<std::string> args, Client &receiver) {
    ...
    // Get the name, asset filename, start value, and time active from the
    // arguments
...
void CloseCommand::handle(std::vector<std::string> args, Client &receiver) {
    ...
    if (!receiver._user.isLoggedIn()) {  // If the user is not logged in, show a
                                         // message and return
        Message::UserNotLoggedIn();
        return;
    }

    // Create a CloseAuctionCommunication object and set the username, password,
    // and AID
...
void ListUserAuctionsCommand::handle(std::vector<std::string> args,
    ...
    receiver.processRequest(
        listUserAuctionsCommunication);  // Send the request to the server,
...
void ListUserBidsCommand::handle(std::vector<std::string> args,
    ...
    receiver.processRequest(
        listUserBidsCommunication);  // Send the request to the server,
                                     // receiving its response
...
void BidCommand::handle(std::vector<std::string> args, Client &receiver) {
    ...
    // Create a BidCommunication object and set the username, password, AID, and
    // value
...
void ShowRecordCommand::handle(std::vector<std::string> args,
    ...
    receiver.processRequest(
        showRecordCommunication);  // Send the request to the server, receiving
                                   // its response
