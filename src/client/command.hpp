...
class CommandHandler {
    public:
        virtual void handle(std::vector<std::string> args, ClientState receiver) = 0;
        std::string _name;
        std::string _description;
        std::string _usage;
        std::vector<std::string> _alias;
    protected:
        CommandHandler(std::string name, std::string description, std::string usage, std::vector<std::string> alias) :
...
class LoginCommand : public CommandHandler {
    public:
        ...
        LoginCommand() : CommandHandler("login", "Logins into the server", "login UID password", std::vector<std::string>({"ln"})) {};
};
